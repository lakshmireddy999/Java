...
fun main(args:Array<String>)
{
    ...
    /*A.1 int type array(implicit declaration) accesing
    using range where i deals with index*/
    println("int type array accesing using range")
    var arr=arrayOf(1,2,3,4,5)  //implicit type declaration. var will seeing the type of data consider it int type array
    for(i in 0..arr.size-1)
    println("array elements are  ${arr[i]}");   // putting semi colon at the end is optional
    println()



    /*A.2. char type array (explicit declaration)accesing
     through for loop over array*/
    println("char type array accesing through elements")
    var arr1=arrayOf<Char>('a','b','c','d')//explicit type declaration using angular bracket telling the compiler that it will take char type data
    for(a in arr1)
    {
    ...
    println("int type array cretaed using buint in function and" +
            "accesing through index of array")
    ...
    println("array created using Array class have elements ${arr3[i]}")
